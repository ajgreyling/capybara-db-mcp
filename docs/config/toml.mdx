...
2. **`[[tools]]`** - Tool configuration (execution settings for execute_sql, search_objects)
...
npx capybara-db-mcp --transport http --port 8080

# Or specify custom location
npx capybara-db-mcp --config=/path/to/config.toml
```
...
<ParamField path="schema" type="string">
  Default schema for this source. (capybara-db-mcp fork.) Same behavior as the [`--schema`](/config/command-line#--schema) command-line flag.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | string | ✅ | Tool name (`execute_sql` or `search_objects`) |
| `source` | string | ✅ | Source ID to use |
| `readonly` | boolean | ❌ | Read-only mode (execute_sql only; default: `false`) |
| `max_rows` | number | ❌ | Max rows limit (execute_sql only; default: unlimited) |
